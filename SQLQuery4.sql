...
CREATE VIEW view_product_order_jianyu
AS (
...
CREATE PROC sp_product_order_quantity_jianyu
@id int,
@quantity int out
AS
BEGIN
...
CREATE PROC sp_product_order_city_jianyu
@name nvarchar(40),
...
/*   Create 2 new tables “people_your_last_name” “city_your_last_name”. 
	 ...
	 If any error occurred, no jianyuges should be made to DB. (after test) Drop both tables and view.	*/
CREATE TABLE city_jianyu (
	Id INT PRIMARY KEY,
	City VARCHAR(50) NOT NULL
)
CREATE TABLE people_jianyu (
	Id INT PRIMARY KEY,
	Name VARCHAR(50) NOT NULL,
	City INT FOREIGN KEY REFERENCES city_jianyu (Id)
)
INSERT INTO city_jianyu VALUES (1, 'Seattle')
INSERT INTO city_jianyu VALUES (2, 'Green Bay')
INSERT INTO people_jianyu VALUES (1, 'Aaron Rodgers', 2)
INSERT INTO people_jianyu VALUES (2, 'Russell Wilson', 1)
INSERT INTO people_jianyu VALUES (3, 'Jody Nelson', 2)

INSERT INTO city_jianyu VALUES (3, 'Madison')
UPDATE people_jianyu SET City = (SELECT Id FROM city_jianyu WHERE City = 'Madison')
WHERE City = (SELECT Id FROM city_jianyu WHERE City = 'Seattle')
DELETE city_jianyu
WHERE City = 'Seattle'

CREATE VIEW packers_jianyu
AS (
	SELECT Id, Name
	FROM people_jianyu
	WHERE City = (SELECT Id FROM city_jianyu WHERE City = 'Green Bay')
)

DROP TABLE people_jianyu
DROP TABLE city_jianyu
DROP VIEW packers_jianyu
...
*/
CREATE PROC sp_birthday_employees_jianyu
AS
BEGIN
	SELECT EmployeeID, LastName, FirstName, BirthDate INTO birthday_employees_jianyu
	FROM Employees WHERE MONTH(BirthDate) = 2
END

EXEC sp_birthday_employees_jianyu

DROP TABLE birthday_employees_jianyu
DROP PROC sp_birthday_employees_jianyu
